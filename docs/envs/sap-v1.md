...
`envs/policyclash_envs/sap.py` in a local `policy-clash` checkout, on
branch `sap-v1-env`, registered as `sap-v1` and passing 19 env-specific
tests (59 total with connect4 and tron-duel's own suites). This doc was
written before that code existed, the way `tron-duel-v1.md` was written
before `tron_duel.h` — and, as expected, writing the C core against it
caught two real gaps in the numbers below, now corrected in place rather
than left to silently disagree with the code (see the two callouts marked
**corrected during implementation**). See `SAP_clone.md` for the full game
rules reference this scopes down from, and `data/turtle_pack/` for the
scraped source data this spec's numbers are drawn from.
...
- [`SAP_clone.md`](../../SAP_clone.md) — full game rules reference, MVP
  scope (§8), and the open questions this spec resolves a few of (§7 and
  the open-questions section).
- [`data/turtle_pack/`](../../data/turtle_pack/) — scraped pet/food data
  this spec's numbers are drawn from.
- `docs/adding-an-env.md` (in the `policy-clash` repo) — the interface,
  the layering, and the rules it enforces.
- `docs/envs/tron-duel-v1.md` (in the `policy-clash` repo) — the
...
- `policy-clash/envs/csrc/sap_binding.c` — the CPython binding. Exports
  ...
  same convention as `tron_duel_binding.c`.
- `policy-clash/envs/policyclash_envs/sap.py` — the adapter, class `Sap`.
  Registered as `sap-v1`.
- `policy-clash/envs/tests/test_sap.py` — 19 tests: spec/registry shape,
  buy/sell/combine/reroll/reposition/food mechanics, illegal-action and
  masked-illegal forfeits, the ended-seat-argument-ignored rule, the
  action-budget-forces-end bound, seed determinism, and one
  hand-computable single-winner battle (Otter outlasts Horse) as a
