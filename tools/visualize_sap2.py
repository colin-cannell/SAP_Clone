...
result - not a fabricated blow-by-blow battle animation. The browser
"Turtle Shop" artifact's battle log is a *separate* JS reimplementation
of the battle algorithm that does track a trace for exactly this reason;
this tool intentionally doesn't duplicate that here, so what it shows is
never at risk of silently drifting from what the real engine did.

Usage (run with the policy-clash venv's python - policyclash_envs is
installed editable there):
    ...
    policy-clash/envs/.venv/bin/python tools/visualize_sap2.py --style random
"""
