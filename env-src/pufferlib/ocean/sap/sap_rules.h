 ...
 *
 * VERBATIM COPY of policy-clash/envs/csrc/sap.h (the policy-clash repo's
 * own rules core, sap-v1). Copied rather than shared via a build-system
 * link because the two are separate git checkouts with no publishing
 * mechanism between them yet - see ocean/sap/sap.h for why this could be
 * copied unchanged at all: this file has zero PufferLib dependency, same
 * as it has zero Python dependency, so the *only* new code this port
 * needed is a binding around these same functions, not a second
 * implementation of them. Re-sync by re-copying if sap.h changes upstream;
 * nothing in this file is PufferLib-specific.
 *
 ...
 * tier progression - see docs/envs/sap-v1.md (in the sap-gym-env project
 * this was designed alongside) for the full design rationale.
 *
